...
function AddButton(props: ComponentProps<'button'>) {
    return (
        <button
            {...props}
            className="rounded p-2 text-text-muted transition-colors hover:text-amber-bright"
        >
            <PlusIcon size={16} />
        </button>
    )
...
function SortableTask({
    ...
    return (
        <div
            ...
            className="group/task relative flex items-center gap-3 rounded border border-border bg-surface-raised px-4 py-3 transition-colors hover:border-border-strong data-[dragging=true]:opacity-30"
        >
            ...
            {isEditing ? (
                <input
                    ...
                    className="flex-1 bg-transparent text-sm leading-relaxed tracking-wide text-text-primary outline-none"
                />
            ) : (
                <span
                    onClick={onStartEdit}
